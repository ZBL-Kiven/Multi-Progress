...
import com.zj.multiProgress.LogUtils
import com.zj.multiProgress.DEFAULT_I
import com.zj.multiProgress.HANDLE_ABANDON
import com.zj.multiProgress.HANDLE_ERROR_SERVICE_DESTROYED
...
internal object ServerBridge {
    ...
    private val serviceConn = object : ServiceConnection {
        ...
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            LogUtils.log("On server service connected")
    ...
    fun bindWebViewService(context: Context, target: String, onServiceBind: (Boolean) -> Unit) {
        ...
        val intent = Intent(RemoteService.ACTION_NAME)
    ...
    fun onServiceDestroyed() {
        ...
        nextBind?.get()?.let { bi ->
            ...
            LogUtils.log("On server service disconnected")
    ...
    internal fun destroy(isStart: Boolean = false) {
        isDestroyed = true
        try {
            LogUtils.log("unbind server service and disconnecting")
            context?.get()?.unbindService(serviceConn)
        } catch (e: Exception) {
            if (!isStart) Log.e("destroy Server-Bridge", "destroy: unbind server service error case : ${e.message}")
